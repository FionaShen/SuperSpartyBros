...
public class CharacterController2D : MonoBehaviour {
	...
	void Update()
	{
		...
		else if (_canDoubleJump && CrossPlatformInputManager.GetButtonDown("Jump")) { // if canDoubleJump and jump button pressed
			DoJump();
		...
		if (CrossPlatformInputManager.GetButtonDown ("Fire1")) {
			if (hasBullet && bullet) {
				...
				if (_facingRight) {
					newBullet = Instantiate (bullet, transform.position + transform.right, transform.rotation) as GameObject;
					newBullet.GetComponent<ConstantForce2D> ().force = new Vector2(transform.right.x * bulletSpeed, 0.0f);
				} else {
					newBullet = Instantiate (bullet, transform.position - transform.right, transform.rotation) as GameObject;
					newBullet.GetComponent<ConstantForce2D> ().force = new Vector2(-transform.right.x * bulletSpeed, 0.0f);
				}
				...
				if (shootSFX) {
					AudioSource.PlayClipAtPoint (shootSFX, newBullet.transform.position, 0.2f);
				}
			}
		}
	}
