...
namespace UnityStandardAssets._2D
{
    public class Camera2DFollow : MonoBehaviour
    {
		...
		private void Update()
        {
			...
			Vector3 aheadTargetPos = new Vector3(target.position.x + m_LookAheadPos.x, transform.position.y, m_OffsetZ );
				
			Vector3 newPos = Vector3.SmoothDamp (transform.position, aheadTargetPos, ref m_CurrentVelocity, damping);
			
			if (newPos.x >= 0 && newPos.x <= 150) { // keep camera within range to avoid show boudaries
